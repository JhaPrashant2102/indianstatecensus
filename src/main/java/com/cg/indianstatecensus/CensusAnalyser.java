...
public class CensusAnalyser<E> {
	...
	public String populationWiseSortedCensusDataInJsonFormat(String csvFilePath) throws CensusAnalyserException {
		...
		List<IndiaCensusCSV> sortedList = censusList.stream()
				.sorted(Comparator.comparing(IndiaCensusCSV::getPopulation).reversed()).collect(Collectors.toList());
		String sortedJsonData = new Gson().toJson(sortedList);
		return sortedJsonData;
	}
	...
	public String populationDensityWiseSortedCensusDataInJsonFormat(String csvFilePath) throws CensusAnalyserException {
		...
		List<IndiaCensusCSV> sortedList = censusList.stream()
				.sorted(Comparator.comparing(IndiaCensusCSV::getDensity).reversed()).collect(Collectors.toList());
		String sortedJsonData = new Gson().toJson(sortedList);
		return sortedJsonData;
	}

	public String areaWiseSortedCensusDataInJsonFormat(String csvFilePath) throws CensusAnalyserException {
		List<IndiaCensusCSV> censusList = (List<IndiaCensusCSV>) getCSVFileList(csvFilePath, IndiaCensusCSV.class);
		if (censusList.size() == 0 || censusList == null) {
			throw new CensusAnalyserException("Empty List", ExceptionType.EMPTY_LIST);
		}
		List<IndiaCensusCSV> sortedList = censusList.stream()
				.sorted(Comparator.comparing(IndiaCensusCSV::getArea).reversed()).collect(Collectors.toList());
		String sortedJsonData = new Gson().toJson(sortedList);
		return sortedJsonData;
	}
